...
- 3 - left
*)
let rec solve network row col direction result =
  ...
  begin
    ...
    match direction with
      | 0 -> solve network (row - 1) col direction result
      | 1 -> solve network row (col + 1) direction result
      | 2 -> solve network (row + 1) col direction result
      | 3 -> solve network row (col - 1) direction result
...
let () =
  ...
  print_string (solve network 0 start 2 ""); print_endline "";
